...
document.addEventListener("DOMContentLoaded", function () {
  ...
  btnSubmit.addEventListener("click", function () {
    ...
    var alertMessage =
      "Mr. " + customerName + ", your request is submitted.\n\n";
...
const viewitemsh = futcards.map((item) => {
  return `
    ...
    <p><img id="star" src="./../public/src//star.svg">&nbsp ${item.rate}</p>
    <p>${item.title}</p>
    <p>${item.des}</p>
...
function populateProductList(productArray) {
  ...
  productArray.forEach((product) => {
    ...
    productDiv.innerHTML = `
            ...
            <p><img id="star" src="./../public/src//star.svg">&nbsp ${product.rate}</p>
            <p>${product.title}</p>
            <p>${product.des}</p>
...
function checkCartStatus(productId) {
  ...
  if (cartItem && cartItem.quantity > 0) {
    ...
    document.getElementById(`viewInCartBtn${productId}`).style.display =
      "inline";
  } else {
    document.getElementById(`addToCartBtn${productId}`).style.display =
      "inline";
...
function updateSidebar() {
  ...
  if (cartItems.length === 0) {
    ...
    emptyCartMessage.innerHTML = `
        <div class="emptycart">
        <img id="cart_svg" src="./../public/src//g3.svg"></img>
      ...
      return `
        ...
        <div class="cd_wraper">
            <div class="cart_div_item">
                ...
                <p class="total_price" ><i class="uil uil-rupee-sign"></i>${
                  item.item.price * item.quantity
                }</p>
                <div class="counter_item">
                 
                    <button class="minus_cart" onclick="decrementQuantity(${
                      item.item.id
                    })" ${item.quantity === 1 ? "disabled" : ""} style="${
        item.quantity === 1 ? "opacity: 0.2; cursor: not-allowed;" : ""
      }">-</button>
                    <p class="dis_cart" >${item.quantity}</p>
                  
                    <button class="plus_cart" onclick="incrementQuantity(${
                      item.item.id
                    })" ${item.quantity === 12 ? "disabled" : ""} style="${
        item.quantity === 12 ? "opacity: 0.2; cursor: not-allowed;" : ""
      }">+</button>
                </div>
                <button onclick="removeFromCart(${
                  item.item.id
                })" class="timesx"><i class="uil uil-trash-alt"></i></button>
            </div>
        </div>
    </div>
        `;
...
function checkofun(tpoio) {
  ...
  const disco = Math.round(tpoio * 0.2);
  dicty.value = `${disco}`;
...
function openPopup() {
  ...
  document.getElementById("editApartment").value =
    document.getElementById("apartment").innerText;
